...
def create_react(llm):
    #import sys, os
    #sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), './src/react-code')))
    from tools import code_exec, code_retrieve
...
async def main():
    ...
    try:
        input_json = json.load(open(prompt_path, "r")).get(args.task)
        ...
        input_data += f"\nThe code languange is {input_lang}.\nAll output files must save in the path: {result_path}/{str(args.lab)}/{str(args.fw)}/{str(args.model)}/{str(args.task)}/agent_output"
