# poissonVI
## chromatin accessibility analysis with poissonVI
## The import form of the PoissonVI package
import scvi
## setup data function of PoissonVI
scvi.external.POISSONVI.setup_anndata(adata, layer=None, batch_key=None, labels_key=None, size_factor_key=None, categorical_covariate_keys=None, continuous_covariate_keys=None, **kwargs)
...
## setup model function of PoissonVI
scvi.external.POISSONVI(adata, n_hidden=None, n_latent=None, n_layers=2, dropout_rate=0.1, latent_distribution='normal', **model_kwargs)
### the detail description of parameters:
...
8. weight_decay (float (default: 0.001)) – weight decay regularization term for optimization
9. eps (float (default: 1e-08)) – Optimizer eps
10. early_stopping (bool (default: True)) – Whether to perform early stopping with respect to the validation set.
11. early_stopping_patience (int (default: 50)) – How many epochs to wait for improvement before early stopping
12. save_best (bool (default: True)) – DEPRECATED Save the best model state with respect to the validation loss (default), or use the final state in the training procedure
13. check_val_every_n_epoch (int | None (default: None)) – Check val every n train epochs. By default, val is not checked, unless early_stopping is True. If so, val is checked every epoch.
14. n_steps_kl_warmup (int | None (default: None)) – Number of training steps (minibatches) to scale weight on KL divergences from 0 to 1. Only activated when n_epochs_kl_warmup is set to None. If None, defaults to floor(0.75 * adata.n_obs).
15. n_epochs_kl_warmup (int | None (default: 50)) – Number of epochs to scale weight on KL divergences from 0 to 1. Overrides n_steps_kl_warmup when both are not None.
