...
AUTOGEN_RUN_PATH="/data/yangliu/bioagent-benchmark/src/autogen-code/run.py" # Path to the autogen run script
...
for KEY in "${KEYS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${gpu_id} python ./run_workflow/autogen_run_workflow.py --task $KEY --model $MODEL --lab $LAB --prompt_path $PROMPT_JSON --result_path $RESULT_PATH --autogen_run_path $AUTOGEN_RUN_PATH
done