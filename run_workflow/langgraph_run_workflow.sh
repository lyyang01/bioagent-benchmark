...
LANGGRAPH_RUN_PATH="/data/yangliu/bioagent-benchmark/src/langgraph-code/run.py" # Path to the autogen run script
...
for KEY in "${KEYS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${gpu_id} python ./run_workflow/langgraph_run_workflow.py --task $KEY --model $MODEL --lab $LAB --prompt_path $PROMPT_JSON --result_path $RESULT_PATH --langgraph_run_path $LANGGRAPH_RUN_PATH
done