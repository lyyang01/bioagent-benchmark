...
tell application "Terminal"
    do script "conda activate llm-agent\npython /Users/liuyang/home/autogen/Autogen-UI-main/coding/{filename}"
end tell
'''
        #cmd = [
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            #    text=True,
            #)
            result = subprocess.Popen(cmd, cwd=work_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            
            try:
                #result = future.result(timeout=timeout) #execute the code
                result_stderr = ""
                with result.stderr:
                    for line in iter(result.stderr.readline, b""):
                        tt = line.strip().decode("utf-8")
        ...
        if result.returncode:
            ...
            #logs = result.stdout
            logs = result.stdout.read().decode("utf-8")
