...
class JupyterClient:
    ...
    def list_kernel_specs(self) -> Dict[str, Dict[str, str]]:
        proxy = {"https":"http://127.0.0.1:33210", "http": "http://127.0.0.1:33210"}
        #import pdb
        #pdb.set_trace()
        response = self._session.get(f"{self._get_api_base_url()}/api/kernelspecs", headers=self._get_headers())
        #use proxy
        #response = self._session.get(f"{self._get_api_base_url()}/api/kernelspecs", headers=self._get_headers(), proxies=proxy)
        #import pdb
        #pdb.set_trace()
...
class JupyterKernelClient:
    ...
    def __init__(self, websocket: WebSocket):
        ...
        #self._websocket.ping = 10000000000
