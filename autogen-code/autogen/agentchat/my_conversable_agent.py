...
class MyConversableAgent(LLMAgent):
    ...
    def __init__(
        ...
        self.register_reply([Agent, None], MyConversableAgent.generate_oai_reply)
        self.register_reply([Agent, None], MyConversableAgent.a_generate_oai_reply, ignore_async_in_sync_chat=True)
        ...
        if code_execution_config is not False:
            ...
            if self._code_execution_config.get("executor") is not None:
                ...
                self.register_reply([Agent, None], MyConversableAgent._generate_code_execution_reply_using_executor)
            else:
                ...
                self.register_reply([Agent, None], MyConversableAgent.generate_code_execution_reply)
        else:
            # Code execution is disabled.
            self._code_execution_config = False

        self.register_reply([Agent, None], MyConversableAgent.generate_tool_calls_reply)
        self.register_reply([Agent, None], MyConversableAgent.a_generate_tool_calls_reply, ignore_async_in_sync_chat=True)
        self.register_reply([Agent, None], MyConversableAgent.generate_function_call_reply)
        self.register_reply(
            [Agent, None], MyConversableAgent.a_generate_function_call_reply, ignore_async_in_sync_chat=True
        )
        self.register_reply([Agent, None], MyConversableAgent.check_termination_and_human_reply)
        self.register_reply(
            [Agent, None], MyConversableAgent.a_check_termination_and_human_reply, ignore_async_in_sync_chat=True
        )
    ...
    def register_reply(
        ...
        *,
        ...
        chats (initiated with `MyConversableAgent.a_initiate_chat`). If an `async` reply function is registered
    ...
    def receive(
        ...
        add_terminate = False
        temp_contents = []
        for ii in self._oai_messages[sender]:
            temp_contents.append(ii['content'])
        if message in temp_contents:
            add_terminate = True
        '''
        else:
            import pdb
        ...
        if add_terminate or ("feel free to ask" in reply) or (len(self._oai_messages[sender])==10):
            reply = reply + "\n\n" + "TERMINATE"
    ...
    def _raise_exception_on_async_reply_functions(self) -> None:
        ...
        if async_reply_functions != []:
            msg = (
                "Async reply functions can only be used with MyConversableAgent.a_initiate_chat(). The following async reply functions are found: "
        ...
        Args:
            ...
            message (str, dict or Callable): the initial message to be sent to the recipient. Needs to be provided. Otherwise, input() will be called to get the initial message.
                ...
                - If a callable is provided, it will be called to get the initial message in the form of a string or a dict. If the returned value is a dict, it should contain the following reserved fields:
                    ...
                    ```python
                    def my_message(sender: MyConversableAgent, recipient: MyConversableAgent, context: dict) -> Union[str, Dict]:
    ...
    def _relfection_with_llm_as_summary(sender, recipient, summary_args):
        ...
        prompt = MyConversableAgent.DEFAULT_SUMMARY_PROMPT if prompt is None else prompt
    ...
    def register_hook(self, hookable_method: str, hook: Callable):
        ...
        Args:
            hookable_method: A hookable method name implemented by MyConversableAgent.
...
def register_function(
    f: Callable[..., Any],
    *,
    caller: MyConversableAgent,
    executor: MyConversableAgent,
    name: Optional[str] = None,
    description: str,
) -> None:
    ...
    Args:
        ...
        caller: the agent calling the function, typically an instance of MyConversableAgent.
