...
Then select the next role from {[agent.name for agent in agents]} to play. Only return the role."""
    ...
    def _prepare_and_select_agents(
        ...
        if isinstance(self.speaker_selection_method, Callable):
            selected_agent = self.speaker_selection_method(last_speaker, self, last_last_speaker, chat_round)
    ...
    def select_speaker(self, last_speaker: Agent, selector: ConversableAgent, last_last_speaker='', chat_round=0) -> Agent:
        """Select the next speaker."""
        selected_agent, agents, messages = self._prepare_and_select_agents(last_speaker, last_last_speaker, chat_round)
        if selected_agent:
    ...
    async def a_select_speaker(self, last_speaker: Agent, selector: ConversableAgent, last_last_speaker='') -> Agent:
        """Select the next speaker."""
        selected_agent, agents, messages = self._prepare_and_select_agents(last_speaker, last_last_speaker)
        if selected_agent:
            return selected_agent
        # auto speaker selection
        selector.update_system_message(self.select_speaker_msg(agents))
...
class GroupChatManager(ConversableAgent):
    ...
    def run_chat(
        ...
        last_speaker_='user_proxy',
        last_last_speaker_='',
    ) -> Tuple[bool, Optional[str]]:
        """Run a group chat."""
        
        last_speaker = last_speaker_
        last_last_speaker = last_last_speaker_
        #problem_set = ["For planner: Why did you choose the specific steps for data preprocessing of the peakVI model, and how do these steps address potential issues in the dataset?", "For planner: How would you adapt the workflow if the peakVI model fails to converge during training?", "For coder: Why did you select specific parameters (e.g., latent dimensions, batch size) for the peakVI model training?", "For coder: When encountering unstable training behaviors (e.g., the loss does not decrease) of the peakVI model, which parameters would you examine, and how would you debug them?", "exit"]
        
        #["For planner: Why did you choose the specific steps for data preprocessing of the contrastiveVI model, and how do these steps address potential issues in the dataset?", "For planner: How would you adapt the workflow if the contrastiveVI model fails to converge during training?", "For coder: Why did you select specific parameters (e.g., latent dimensions, batch size) for the contrastiveVI model training?", "For coder: When encountering unstable training behaviors (e.g., the loss does not decrease) of the contrastiveVI model, which parameters would you examine, and how would you debug them?", "exit"]
        
        #["For planner: Why did you choose the specific steps for data preprocessing of the cellassign model, and how do these steps address potential issues in the dataset?", "For planner: How would you adapt the workflow if the cellassign model fails to converge during training?", "For coder: Why did you select specific parameters (e.g., latent dimensions, batch size) for the cellassign model training?", "For coder: When encountering unstable training behaviors (e.g., the loss does not decrease) of the cellassign model, which parameters would you examine, and how would you debug them?", "exit"]
        
        #["For planner: Why did you choose the specific steps for data preprocessing of the scVI model, and how do these steps address potential issues in the dataset?", "For planner: How would you adapt the workflow if the scVI model fails to converge during training?", "For coder: Why did you select specific parameters (e.g., latent dimensions, batch size) for the scVI model training?", "For coder: When encountering unstable training behaviors (e.g., the loss does not decrease), which parameters would you examine, and how would you debug them?", "exit"]

        #["For planner: Why did you choose the specific steps for data preprocessing of scVI model, and how do these steps address potential issues in the dataset?", "For planner: How would you adapt the workflow if the scVI model fails to converge during training?", "For coder: ", "exit"]
        ...
        for i in range(groupchat.max_round):
            ...
            if self._is_termination_msg(message) or i == groupchat.max_round - 1:
                ...
                break
            if last_speaker == "terminator":
                break
            try:
                # select the next speaker
                speaker = groupchat.select_speaker(speaker, self, last_last_speaker, correction_round)
                #if speaker.name == 'planner':
                #    import pdb
                #    pdb.set_trace()
                #if speaker.name == 'coder':
                    #TODO judge whether two same errors occur
                #    if len(reply) > 3000:
                        #reply = reply[0:800] + '\n' + reply[-8000:]
                        #tmp_ = reply.split("\n")[-1]
                #        reply = reply[0:3000]
                ...
                try:
                    ...
                    if "Broken pipe" in str(e):
                        reply = "exitcode: 1. Execute the code failed. Occur broken pipe error. Please check the code and try again."
                #if speaker.name == "coder":
                #    import pdb
                #   pdb.set_trace()
                #if speaker.name == "chat_manager":
                #    if len(reply) > 10000:
                        #reply = reply[0:800] + '\n' + reply[-8000:]
                        #tmp_ = reply.split("\n")[-1]
                #        reply = reply[0:10000]
                if speaker.name == 'manager':
                    #if "TERMINATE" not in reply:
                    if reply == last_manager_content:
                        reply = "The agent workflow seems to be stuck in a loop. Let's invoke the Terminator to end the process.\nTERMINATE"
                    else:
                    #add previous codes
                        last_manager_content = reply
                    '''
                    if '```python' in reply:
                        pass
                    elif ('TERMINATE' not in reply) and (last_speaker != 'user_proxy2'): # not the last step
                        if len(speaker_message_list['executor']) > 0:
                            tmp = speaker_message_list['executor'][-1]
                            #extract code block
                            #if "```python" in tmp:
                            #import pdb
                            #pdb.set_trace()
                            ...
                            previous_codes_prompts = '\n'.join(coder_messages)
                        try:
                            if "Codes of previous steps are as the following:" in reply:
                                pass
                            else:
                                reply = reply + '\n\nCodes of previous steps are as the following:\n' + previous_codes_prompts
                            
                        except:
                            pass
                    '''
                    #reply = reply + "\n\nplease retrieve content"
                    #if "Step 2" in reply and "Step 3" not in reply:
                    #    reply = reply + "\n\nplease retrieve content"
                
                if speaker.name == 'terminator':
                    reply = "exit"
                ...
                if speaker.name == 'coder':
                    ...
                    #if len(speaker_message_list['executor'])>0:
                        if correction_round > 4:#4
                            reply = "The number of correction rounds has already reached five, but errors still occur in execution. The multi-agent system is about to exit.\n\nTERMINATE"
                        if ("exitcode: 0" not in speaker_message_list['executor'][-1]) and (speaker_message_list['executor'][-1] in speaker_message_list['executor'][0:-1]):
                            reply = "The code execution encountered the same error twice, and I am unable to resolve it independently. The multi-agent system will exit.\n\nTERMINATE"
                        

                #if speaker.name == 'user_proxy2':
                #    reply = problem_set[0]
                ...
                if speaker.name == 'executor':
                    if reply is not None:
                        ...
                        if len(reply) > 2000:
                            ...
                            tmp_ = reply.split("\n")[-1]
                            reply = reply[0:2000] + "\n" + tmp_
                        if "exitcode: 1" in reply:
                            correction_round += 1
                        elif "exitcode: 0" in reply:
                            correction_round = 0
                        #TODO 
                        #judge whether two same errors occur
                        #if reply in speaker_message_list[speaker.name]:
                        #    reply = reply + "\n\nThe code execution encountered the same error twice, and we are unable to resolve it independently. The multi-agent system will exit."
                    
                speaker_message_list[speaker.name].append(message['content'])
                #TODO 2025/05/31
                #if last_reply.endswith("**TERMINATE**") or last_reply.endswith("TERMINATE"):
                #    break
            ...
            #last_reply = reply
            if reply is None:
                #reply =  "The reply is None. Please regenerate the reply!"
            ...
            #TODO
            last_last_speaker = last_speaker
    ...
    async def a_run_chat(
        ...
        for i in range(groupchat.max_round):
            ...
            try:
                # select the next speaker
                speaker = await groupchat.a_select_speaker(speaker, self, last_last_speaker)
                # let the speaker speak
