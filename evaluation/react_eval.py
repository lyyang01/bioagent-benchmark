from evaluation_function import monitor_process, monitor_process_code, task_completion_react, task_completion_react_noplan, round_statics_m2, plan_score, code_score, rag_eval, rag_eval_noplan, consistency_with_plan
import pandas as pd
import os
import json
...
def plan_eval(all_scores, json_data, tool_name, model):
    plan_str=""    
...
def task_eval(all_scores, json_data, plan_str, root_path, tool_name,lang):
    workflow_str=str(json_data)
    # tc_dict = task_completion(workflow_str)
    tc_dict = task_completion_react(workflow_str, plan_str, root_path ,tool_name,lang)
...
def ragtool_eval_noplan(all_scores, root_path):
    rag_path = os.path.join(root_path, "rag.txt")
    with open(rag_path) as f:
        rag_str = f.read()
    rag_list = eval(rag_str)
    if_retrieval_accuracy, retrieval_accuracy, [total_step, retrieval_step, retrieval_relate_step] = rag_eval_noplan(rag_list, tool_name)
    all_scores["rag"]["if_retrieval_accuracy"] = eval(if_retrieval_accuracy[0:-1])/100
    all_scores["rag"]["retrieval_accuracy"] = retrieval_accuracy
    return all_scores
...
# all_scores[tool_name] = {"collaboration":{}, "execution":{}, "plan":{}, "code":{}, "rag":{}}
...
if args.monitor:
    ...
    if lab in ["ablation/without_planning"]:
        ...
        all_score, plan_str = plan_eval(all_score, json_data, tool_name, model_name)

        all_score, step_number = task_eval(all_score, json_data, lab, plan_str, root_path, tool_name, args.lang)
