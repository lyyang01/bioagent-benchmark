#!/usr/bin/env python3
import sys, os
#sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '/data/yangliu/bioagent-benchmark/evaluation/langgraph/')))
import json
import subprocess
import sys
import os
...
def main():
    ...
    #file_path = f"{result_path}/{lab}/{frame_work}/{model}/{frame_work}_{model}_scores.json"
    ...
    for task_key, task_info in task_data.items():
        ...
        cmd = (
            f"conda run -n '{conda_env}' python evaluation/langgraph/langgraph_eval.py "
            f"--model '{model}' "
            f"--tool '{task_key}' "
            f"--lang '{lang}' "
            f"--fw {frame_work} "
