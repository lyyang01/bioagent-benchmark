...
from evaluation_function import monitor_process_code, task_completion_react, task_completion_react_noplan, round_statics_m2, plan_score, code_score, rag_eval, rag_eval_noplan, consistency_with_plan_lang
import pandas as pd
import os
import json
...
def plan_eval(all_scores, json_data, tool_name, model, prompt_path, eval_model):
    plan_str=""    
    for entry in json_data:
        if entry.get("role") == "assistant" and entry.get("name") == "react_agent":
            try:
                parsed_content = json.loads(entry.get("content", ""))
                if "agent" in parsed_content and "messages" in parsed_content["agent"]:
                    if model != "deepseek-r1":
                        message = parsed_content["agent"]["messages"][0]
                        if "plan for" in message["content"].lower() and "step 1" in message["content"].lower():
                            plan_str = message["content"]
                            break
                    else:
                        message = parsed_content["agent"]["messages"] 
                        if "plan for" in str(message).lower() or "step" in str(message).lower():
                            plan_str = message                
                            break
            except:
                continue
...
def task_eval(all_scores, json_data, plan_str, root_path, tool_name,lang, eval_model):
    workflow_str=str(json_data)
    # tc_dict = task_completion(workflow_str)
    tc_dict = task_completion_react(workflow_str, plan_str, root_path ,tool_name,lang, eval_model)
...
def ragtool_eval_noplan(all_scores, root_path, database_path, eval_model):
    rag_path = os.path.join(root_path, "rag.txt")
    with open(rag_path) as f:
        rag_str = f.read()
    rag_list = eval(rag_str)
    if_retrieval_accuracy, retrieval_accuracy, [total_step, retrieval_step, retrieval_relate_step] = rag_eval_noplan(rag_list, tool_name, database_path, eval_model)
    all_scores["rag"]["if_retrieval_accuracy"] = eval(if_retrieval_accuracy[0:-1])/100
    all_scores["rag"]["retrieval_accuracy"] = retrieval_accuracy
    return all_scores
...
if lab in ["ablation/without_planning"]:
    ...
    tc_dict = task_completion_react_noplan(json_data)
    all_score["execution"]["success_rate"] = tc_dict["success_rate"]

    all_score = round_eval(all_score, json_data, 1)
    all_score = ragtool_eval_noplan(all_score, root_path)

    pass

else:
    all_score, plan_str = plan_eval(all_score, json_data, tool_name, model_name, json_path, eval_model)

    all_score, step_number = task_eval(all_score, json_data, plan_str, root_path, tool_name, args.lang, eval_model)
